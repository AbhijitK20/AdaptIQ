...
import { AlertTriangle, ArrowRight, GitBranch, Lightbulb, BookOpen, Network } from 'lucide-react';
...
export function DiagnosisPanel({ missingConcept, dependencyChain, suggestion }: DiagnosisPanelProps) {
  const primaryPrerequisite = dependencyChain[0] ?? missingConcept;

  return (
    <Card className="pixel-panel border-[#F59E0B]/30 bg-[#F59E0B]/5">
      ...
      <CardContent className="space-y-4">
        ...
        {/* Action Buttons */}
        <Link href={`/knowledge-map?subject=${encodeURIComponent(primaryPrerequisite.subject)}&concept=${primaryPrerequisite.id}`}>
          <Button variant="secondary" className="w-full gap-2">
            <Network className="h-4 w-4" />
            View Previous Prerequisites
          </Button>
        </Link>
        <Link href={`/practice?subject=${encodeURIComponent(missingConcept.subject)}&chapter=${encodeURIComponent(missingConcept.chapter)}&concept=${missingConcept.id}&returnTo=knowledge-map`}>
